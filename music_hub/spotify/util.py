...
def update_or_create_token(session_id, access_token, refresh_token, expires_in, token_type):
    ...
    if tokens and expires_in != None:
        ...
        tokens = SpotifyToken(user=session_id, access_token=access_token, refresh_token=refresh_token, expires_in=timezone.now() + timedelta(seconds=expires_in), token_type=token_type)
        tokens.save() 
