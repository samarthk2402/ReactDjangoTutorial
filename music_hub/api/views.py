...
class GetRoom(APIView):
    serializerClass = RoomSerializer
...
class JoinRoom(APIView):
    ...
    def post(self, request, format=None):
        ...
        if code != None:
            if len(Room.objects.filter(code=code)) > 0:
...
class UserInRoom(APIView):
    def get(self, request, format=None):
        ...
        room = Room.objects.filter(host=self.request.session.session_key)
        if len(room) > 0:
            code = room[0].code
...
class LeaveRoom(APIView):
    def post(self, request, format=None):
        ...
        return Response({"message": "Room code removed from session"}, status=status.HTTP_200_OK)
